...
import com.login.Email_PeopleSoft_Failure;
import com.login.Login;
import com.model.ExlData;
import com.util.ConfigReader;
import com.util.ExcelReader;

public class Main_InstanceStatus_Failure {
	ExcelReader excelReaderObj;
	ConfigReader configReaderObj;
	Driver driverObj;
	Login loginObj;
	Email_PeopleSoft_Failure failureEmailObj;
	Properties prop;
	String configFilePath = "D:\\PeopleSoft\\config\\config.properties";
	List<ExlData> excelDataList = new ArrayList<ExlData>();
	WebDriver driver;
	static Logger log = Logger.getLogger(Main_InstanceStatus_Failure.class);

	public Main_InstanceStatus_Failure(){
		excelReaderObj = new ExcelReader();
		configReaderObj = new ConfigReader();
		driverObj = new Driver();
		loginObj = new Login();
		failureEmailObj = new Email_PeopleSoft_Failure();
	}
	...
	void sendEmail() throws AddressException, MessagingException{
		failureEmailObj.configureEmail(prop, excelDataList);
	}
	...
	public static void main(String[] args) throws IOException, InterruptedException, AddressException, MessagingException{
		...
		Main_InstanceStatus_Failure mainObj = new Main_InstanceStatus_Failure();
		mainObj.setup();
		mainObj.login();
		mainObj.sendEmail();
	}
